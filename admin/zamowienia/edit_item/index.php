...
if(isset($_POST['submit'])){
    try {
        ...
        $update = $pdo->prepare("UPDATE order_items SET item_id=:item_id, amount=:amount where order_id='".htmlspecialchars($_GET['order_id'])."' and item_id='".htmlspecialchars($_GET['item_id'])."'");
        $update->execute($data);
...
<div class="container-fluid p-0 d-flex h-100">
    ...
    <div class="bg-light flex-fill table-responsive">
        ...
        <div class="p-4">
            ...
            <?php
                try {
                    $items = $pdo->prepare("SELECT * FROM order_items where order_id='".htmlspecialchars($_GET['order_id'])."' and item_id='".htmlspecialchars($_GET['item_id'])."'");
                    $items->execute();
                }catch (PDOException $e) {
                    echo 'Nie udało się odczytać danych z bazy';
                    //exit();
                ...
                foreach($items as $item){
                    echo '
                        <tr>
                            <td><input type="number" id="item_id" name="item_id" class="form-control" min=1 step="1" value="'.$item['item_id'].'" required></td>
                            <td><input type="number" id="amount" name="amount" class="form-control" min=1 step="1" value="'.$item['amount'].'" required></td>
                        </tr>
                    ';
                }
            ?>
