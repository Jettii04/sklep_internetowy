...
if(isset($_POST['submit'])){
    try {
        ...
        $update = $pdo->prepare("UPDATE orders SET user=:user, order_status=:order_status, payment_method=:payment_method, delivery_method=:delivery_method, postal_code=:postal_code, city=:city, road=:road, house_number=:house_number where order_id='".htmlspecialchars($_GET['order_id'])."'");
        $update->execute($data);
...
<div class="container-fluid p-0 d-flex h-100">
    ...
    <div class="bg-light flex-fill table-responsive">
        ...
        <div class="p-4">
            ...
            <?php
                try {
                    $orders = $pdo->prepare("SELECT * FROM orders where order_id='".htmlspecialchars($_GET['order_id'])."'");
                    $orders->execute();
                }catch (PDOException $e) {
                    echo 'Nie udało się odczytać danych z bazy';
                    //exit();
                ...
                foreach($orders as $order){
                    ...
                    echo'
                        ...
                        ';
                            ...
                            foreach($do as $method){
                                if($method['delivery_id']==$order['delivery_method']){
                                    echo'<option value="'.$method['delivery_id'].'" selected>'.$method['name'].'-'.$method['price'].'zł</option>';
                                }else{
                        ...
                        ';
                            ...
                            foreach($po as $method){
                                if($method['payment_id']==$order['payment_method']){
                                    echo'<option value="'.$method['payment_id'].'" selected>'.$method['name'].'</option>';
                                }else{
                    ...
                    echo'
                            <td><input type="text" id="postal_code" name="postal_code" class="form-control" placeholder="Kod pocztowy..." value="'.$order['postal_code'].'" required></td>
                            <td><input type="text" id="city" name="city" class="form-control" placeholder="Miasto..." value="'.$order['city'].'" required></td>
                            <td><input type="text" id="road" name="road" class="form-control" placeholder="Ulica..." value="'.$order['road'].'"required></td>
                            <td><input type="text" id="house_number" name="house_number" class="form-control" placeholder="Nr. budynku.." value="'.$order['house_number'].'" required></td>
                        </tr>
                    ';
                }
            ?>
