...
if(isset($_POST['submit'])){
    try {
        $update = $pdo->prepare("UPDATE info_sites set header='".htmlspecialchars($_POST['header'])."', body='".htmlspecialchars($_POST['body'])."' where site_id=".htmlspecialchars($_GET['id']));
        $update->execute();
...
<div class="container-fluid p-0 d-flex h-100">
    ...
    <div class="bg-light flex-fill table-responsive">
        ...
        <div class="p-4">
            ...
            <?php
                try {
                    $categories = $pdo->prepare("SELECT * FROM info_sites where site_id=".htmlspecialchars($_GET['id']));
                    $categories->execute();
                }catch (PDOException $e) {
                    echo 'Nie udało się odczytać danych z bazy';
                    //exit();
                ...
                foreach($categories as $category){
                    echo '
                        <tr>
                            ...
                            <td><input type="text" id="header" name="header" class="form-control" placeholder="Nagłówek..." value="'.$category['header'].'" maxlength="50" required></td>
                            <td><textarea id="body" name="body" class="form-control" placeholder="Treść..." required>'.$category['body'].'</textarea></td>
                        </tr>
                    ';
                }
            ?>
