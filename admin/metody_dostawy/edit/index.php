...
if(isset($_POST['name'])){
    try {
        $update = $pdo->prepare("UPDATE delivery_method set name='".htmlspecialchars($_POST['name'])."', price='".htmlspecialchars($_POST['price'])."' where delivery_id=".htmlspecialchars($_GET['id']));
        $update->execute();
...
<head>
    <meta charset="utf-8">
    <title>Metody dostawy - Edytuj</title>
...
<div class="container-fluid p-0 d-flex h-100">
    ...
    <div class="bg-light flex-fill table-responsive">
        ...
        <div class="p-4">
            ...
            <?php
                try {
                    $categories = $pdo->prepare("SELECT * FROM delivery_method where delivery_id=".htmlspecialchars($_GET['id']));
                    $categories->execute();
                }catch (PDOException $e) {
                    echo 'Nie udało się odczytać danych z bazy';
                    //exit();
                ...
                foreach($categories as $category){
                    echo '
                        <tr>
                            ...
                            <td><input type="text" id="name" name="name" class="form-control" placeholder="Nazwa..." value="'.$category['name'].'"></td>
                            <td><input type="text" id="price" name="price" class="form-control" placeholder="Cena w zł..." value="'.$category['price'].'" pattern="[\d]{1,}[\.]{0,1}[\d]{0,2}" required></td>
                        </tr>
                    ';
                }
            ?>
