...
if(isset($_POST['submit'])){
    try {
        $update = $pdo->prepare("UPDATE note set contents='".htmlspecialchars($_POST['contents'])."' where note_id=".htmlspecialchars($_GET['id']));
        $update->execute();
...
<div class="container-fluid p-0 d-flex h-100">
    ...
    <div class="bg-light flex-fill table-responsive">
        ...
        <div class="p-4">
            ...
            <?php
                try {
                    $categories = $pdo->prepare("SELECT * FROM note where note_id=".htmlspecialchars($_GET['id']));
                    $categories->execute();
                }catch (PDOException $e) {
                    echo 'Nie udało się odczytać danych z bazy';
                    //exit();
                ...
                foreach($categories as $category){
                    echo '
                        <tr>
                            <td><textarea type="text" id="contents" name="contents" class="form-control" placeholder="Nazwa...">'.$category['contents'].'</textarea></td>
                        </tr>
                    ';
                }
            ?>
