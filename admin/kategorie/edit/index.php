...
if(isset($_POST['name'])){
    try {
        $update = $pdo->prepare("UPDATE category set name='".htmlspecialchars($_POST['name'])."' where category_id=".htmlspecialchars($_GET['id']));
        $update->execute();
...
<div class="container-fluid p-0 d-flex h-100">
    ...
    <div class="bg-light flex-fill table-responsive">
        ...
        <div class="p-4">
            ...
            <?php
                try {
                    $categories = $pdo->prepare("SELECT * FROM category where category_id=".htmlspecialchars($_GET['id']));
                    $categories->execute();
                }catch (PDOException $e) {
                    echo 'Nie udało się odczytać danych z bazy';
                    //exit();
